document.addEventListener('DOMContentLoaded', async () => {
    ...
    const arrive = document.getElementById('arrival_place');
    arrive.value = location_name;

    const main = document.querySelector('.way_icon');
    const mainStorage = localStorage.getItem('mainHtml')
    if (main && mainStorage) {
        main.innerHTML = mainStorage
    }

    localStorage.removeItem('mainHtml')
    ...
    // 출발지 설정
    const search_depart = document.querySelector('.search_depart')
    search_depart.addEventListener('click', () => {
        const current = document.querySelector('.current_position')
    ...
    // 도착지 설정
    const arrival_search = document.querySelector('.search_arrive')
    arrival_search.addEventListener('click', () => {
        const arrival = document.getElementById('arrival_place')
        const current = document.querySelector('.current_position')
        current.style.display = 'none'
        // 검색하여 마커를 찍은 다음 좌표만 가져오기
        var ps = new kakao.maps.services.Places();
    ...
    const find_road = document.querySelector('.find_road_to_destination')
    let polylineObj = {};
